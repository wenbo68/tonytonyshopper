...
export const orderRouter = createTRPCRouter({
  ...
  approveOrderItemReturn: protectedProcedure
    .input(
      z.object({
        orderItemId: z.string(),
        quantity: z.number().min(1, "Invalid quantity."),
        returnLabel: z.string().min(1, "Invalid return label."),
        returnCarrier: z.string().min(1, "Invalid return carrier."),
        returnTrackingNumber: z
          .string()
          .min(1, "Invalid return tracking number."),
      }),
    )
    .mutation(async ({ ctx, input }) => {
      const {
      ...
      await ctx.db.transaction(async (tx) => {
        await updateOrderItem(
          ...
          "return_approved",
          quantity,
          { returnLabel, returnCarrier, returnTrackingNumber },
        );
      });
    }),

  rejectOrderItemReturn: protectedProcedure
    .input(
      z.object({
        orderItemId: z.string(),
        quantity: z.number().min(1, "Invalid quantity."),
